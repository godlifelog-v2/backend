...
public class PlanAdminController {
  ...
  // ----------- 관리자가 작성한 루틴 리스트 조회 ---------------
  @GetMapping
  public ResponseEntity<?> selectAdminPlanList(
    ...
    if (plans.isEmpty()) {
      return ResponseEntity.status(HttpStatus.NO_CONTENT)
              .body(ApiResponse.of(204, "관리자 루틴이 없습니다."));
    }
    ...
    return ResponseEntity.ok(ApiResponse.of(200, "관리자 루틴 조회 성공", data));
  }

  // ----------- 관리자가 작성한 루틴 카테고리 리스트 조회 ---------------
  @GetMapping("/{targetIdx}")
  public ResponseEntity<?> getAllAdminPlans(
    ...
    if (plans.isEmpty()) {
      return ResponseEntity.status(HttpStatus.NO_CONTENT)
              .body(ApiResponse.of(204, "카테고리별 관리자 루틴이 없습니다."));
    }
    ...
    return ResponseEntity.ok(ApiResponse.of(200, "카테고리별 관리자 루틴 조회 성공", data));
  }
  //    ----------- 상세보기는 해당 url 사용 : api/plan/detail/{planIdx}

  // 전체 조회
  @GetMapping("/all")
  public ResponseEntity<?> selectPlanList(
          @RequestParam(defaultValue = "1") int page,
          @RequestParam(defaultValue = "10") int size) {
    ...
    if (plans.isEmpty()) {
      return ResponseEntity.status(HttpStatus.NO_CONTENT)
              .body(ApiResponse.of(204, "루틴이 없습니다."));
    }
    ...
    return ResponseEntity.ok(ApiResponse.of(200, "루틴 조회 성공", data));
  }
