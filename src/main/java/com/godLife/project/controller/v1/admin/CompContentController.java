...
public class CompContentController {
  ...
  public ResponseEntity<?> createCategory(@RequestBody TargetCateDTO targetCateDTO) {
    try {
      int result = compContentService.insertTargetCategory(targetCateDTO);

      if (result == 409) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(ApiResponse.of(409, "이미 존재하는 카테고리 이름입니다."));
      }
      return ResponseEntity.status(HttpStatus.CREATED)
              .body(ApiResponse.of(201, "목표 카테고리 등록 성공"));

    } catch (Exception e) {
      log.error("목표 카테고리 등록 실패: {}", e.getMessage(), e);
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
              .body(ApiResponse.of(500, "목표 카테고리 등록 중 서버 오류가 발생했습니다."));
    }
  }
  ...
  public ResponseEntity<?> updateCategory(
    ...
    try {
      ...
      if (result > 0) {
        return ResponseEntity.ok(ApiResponse.of(200, "목표 카테고리 수정 성공"));
      } else {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "수정할 카테고리를 찾을 수 없습니다."));
      }

    } catch (Exception e) {
      log.error("목표 카테고리 수정 실패: {}", e.getMessage(), e);
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
              .body(ApiResponse.of(500, "목표 카테고리 수정 중 서버 오류가 발생했습니다."));
    }
  }

  // 목표 카테고리 삭제
  @DeleteMapping("targetCategory/{idx}")
  public ResponseEntity<?> deleteCategory(@PathVariable int idx) {
    try {
      int result = compContentService.softDeleteTargetCategory(idx);

      if (result > 0) {
        return ResponseEntity.ok(ApiResponse.of(200, "목표 카테고리 삭제 성공"));
      } else {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "삭제할 카테고리를 찾을 수 없습니다."));
      }

    } catch (Exception e) {
      log.error("목표 카테고리 삭제 실패: {}", e.getMessage(), e);
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
              .body(ApiResponse.of(500, "목표 카테고리 삭제 중 서버 오류가 발생했습니다."));
    }
  }
  ...
  public ResponseEntity<?> createJobCategory(@RequestBody JobCateDTO jobCateDTO) {
    try {
      int result = compContentService.insertJobCategory(jobCateDTO);
      if (result > 0) {
        return ResponseEntity.ok(ApiResponse.of(200, "직업 카테고리 생성 성공"));
      } else {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ApiResponse.of(400, "직업 카테고리 생성 실패"));
      }
    } catch (Exception e) {
  ...
  public ResponseEntity<?> updateJobCategory(
    ...
    try {
      ...
      if (result > 0) {
        return ResponseEntity.ok(ApiResponse.of(200, "직업 카테고리 수정 성공"));
      } else {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "수정할 카테고리를 찾을 수 없습니다."));
      }
  ...
  public ResponseEntity<?> deleteJobCategory(@PathVariable("jobIdx") int jobIdx) {
    try {
      int result = compContentService.deleteJobCategory(jobIdx);
      if (result > 0) {
        return ResponseEntity.ok(ApiResponse.of(200, "직업 카테고리 삭제 성공"));
      } else {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "삭제할 카테고리를 찾을 수 없습니다."));
      }
  ...
  // 등급(불꽃) 추가
  @PostMapping("fire")
  public ResponseEntity<?> insertFire(@RequestBody FireDTO fireDTO) {
    try {
      int result = compContentService.insertFire(fireDTO);

      if (result == 409) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(ApiResponse.of(409, "이미 존재하는 등급 이름입니다."));
      }

      return ResponseEntity.status(HttpStatus.CREATED)
              .body(ApiResponse.of(201, "등급(불꽃) 등록 성공"));

    } catch (Exception e) {
      log.error("등급(불꽃) 등록 실패: {}", e.getMessage(), e);
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
              .body(ApiResponse.of(500, "등급(불꽃) 등록 중 서버 오류가 발생했습니다."));
    }
  }

  // 등급(불꽃) 수정
  @PatchMapping("/fire/{lvIdx}")
  public ResponseEntity<?> updateFire(
    ...
    try {
      ...
      if (result > 0) {
        return ResponseEntity.ok(ApiResponse.of(200, "등급(불꽃) 수정 성공"));
      } else {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "수정할 등급(불꽃)을 찾을 수 없습니다."));
      }
    } catch (Exception e) {
  ...
  public ResponseEntity<?> deleteFire(@PathVariable("lvIdx") int lvIdx) {
    try {
      int result = compContentService.deleteFire(lvIdx);
      if (result > 0) {
        return ResponseEntity.ok(ApiResponse.of(200, "등급(불꽃) 삭제 성공"));
      } else {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "삭제할 등급(불꽃)을 찾을 수 없습니다."));
      }
    } catch (Exception e) {
  ...
  public ResponseEntity<?> insertChallCate(@RequestBody ChallengeCateDTO challengeCateDTO) {
    try {
      ...
      return ResponseEntity.status(HttpStatus.CREATED)
              .body(ApiResponse.of(201, "챌린지 카테고리 등록 성공"));

    } catch (Exception e) {
      log.error("챌린지 카테고리 등록 실패: {}", e.getMessage(), e);
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
              .body(ApiResponse.of(500, "챌린지 카테고리 등록 중 서버 오류가 발생했습니다."));
    }
  }

  // 챌린지 카테고리 수정
  @PatchMapping("/challCate/{challCategoryIdx}")
  public ResponseEntity<?> updateChallCate(
    ...
    try {
      ...
      if (result > 0) {
        return ResponseEntity.ok(ApiResponse.of(200, "챌린지 카테고리 수정 성공"));
      } else {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "수정할 카테고리를 찾을 수 없습니다."));
      }
  ...
  public ResponseEntity<?> deleteChallCate(@PathVariable("challCategoryIdx") int challCategoryIdx) {
    try {
      int result = compContentService.deleteChallCate(challCategoryIdx);
      if (result > 0) {
        return ResponseEntity.ok(ApiResponse.of(200, "챌린지 카테고리 삭제 성공"));
      } else {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "삭제할 카테고리를 찾을 수 없습니다."));
      }
...
/*
  ...
  public ResponseEntity<?> targetCategoryList() {
    try {
      List<TargetCateDTO> categoryList = compContentService.targetCategoryList();

      if (categoryList.isEmpty()) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "등록된 목표 카테고리가 없습니다."));
      }

      Map<String, Object> response = ApiResponse.of(200, "목표 카테고리 조회 성공");
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
              .body(ApiResponse.of(500, "서버 오류로 인해 카테고리 조회에 실패했습니다."));
    }
  }
...
// 직업 카테고리 조회
  ...
  public ResponseEntity<?> jobCategoryList() {
    try {
      List<JobCateDTO> jobCategoryList = compContentService.getAllJobCategories();

      if (jobCategoryList.isEmpty()) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "등록된 직업 카테고리가 없습니다."));
      }

      Map<String, Object> response = ApiResponse.of(200, "직업 카테고리 조회 성공");
  ...
  // 등급
@GetMapping("fire")
  public ResponseEntity<?> selectAllFireGrades(FireDTO fireDTO) {
    try {
      List<FireDTO> fireDTOList = compContentService.selectAllFireGrades();

      if (fireDTOList.isEmpty()) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "등록된 등급(불꽃)이 없습니다."));
      }

      Map<String, Object> response = ApiResponse.of(200, "등급(불꽃) 조회 성공");
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
              .body(ApiResponse.of(500, "서버 오류로 인해 등급(불꽃) 조회에 실패했습니다."));
    }
  }
  
  // 챌린지
 @GetMapping("challCate")
  public ResponseEntity<?> selectChallCate() {
    try {
      List<ChallengeCateDTO> categoryList = compContentService.selectChallCate();

      if (categoryList.isEmpty()) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ApiResponse.of(404, "등록된 챌린지 카테고리가 없습니다."));
      }

      Map<String, Object> response = ApiResponse.of(200, "챌린지 카테고리 조회 성공");
      response.put("categories", categoryList);
