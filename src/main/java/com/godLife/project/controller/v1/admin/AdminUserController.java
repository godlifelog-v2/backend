...
public class AdminUserController {
        ...
        try {
            ...
            if (users.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NO_CONTENT)
                        .body(ApiResponse.of(204, "조회된 유저가 없습니다."));
            }
            ...
            return ResponseEntity.ok(ApiResponse.of(200, "유저 리스트 조회 성공", result));

        } catch (Exception e) {
            log.error("유저 리스트 페이징 조회 중 오류 발생: ", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.of(500, "서버 오류로 유저 리스트 조회에 실패했습니다."));
        }
    }
    ...
    public ResponseEntity<?> banUser(@PathVariable int userIdx) {
        try {
            if (adminUserService.existsByUserIdx(userIdx) == 0) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(ApiResponse.of(404, "존재하지 않는 유저입니다."));
            }

            int result = adminUserService.banUser(userIdx);

            if (result > 0) {
                int banned = adminUserService.getBanStatus(userIdx);
                ...
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(ApiResponse.of(404, "상태 변경에 실패했습니다."));
            }

        } catch (DataAccessException dae) {
            log.error("DB 오류 발생: ", dae);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.of(500, "DB 오류로 회원 상태 변경에 실패했습니다."));
        } catch (Exception e) {
            log.error("회원 상태 변경 중 오류 발생: ", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.of(500, "서버 오류로 회원 상태 변경에 실패했습니다."));
        }
    }
    ...
    public ResponseEntity<?> getAuthorityList() {
        try {
            ...
            if (authorityList.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(ApiResponse.of(404, "등록된 권한이 없습니다."));
            }

            return ResponseEntity.ok(ApiResponse.of(200, "권한 목록 조회 성공", Map.of("authorityList", authorityList)));
        } catch (Exception e) {
            log.error("권한 목록 조회 실패: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.of(500, "서버 오류로 권한 목록을 불러오지 못했습니다."));
        }
    }
    ...
    public ResponseEntity<?> AdminList(){
        try {
            ...
            if (userList.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(ApiResponse.of(404, "관리자 권한을 가진 유저가 없습니다."));
            }

            return ResponseEntity.ok(ApiResponse.of(200, "권한별 유저 조회 성공", Map.of("userList", userList)));
        } catch (Exception e) {
            log.error("관리자 권한 유저 조회 실패: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.of(500, "서버 오류로 관리자 목록을 불러오지 못했습니다."));
        }
    }
    ...
    public ResponseEntity<?> getUsersByAuthority(@PathVariable int authorityIdx) {
        try {
            List<AdminUserDTO> userList = adminUserService.getUsersByAuthority(authorityIdx);

            if (userList.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(ApiResponse.of(404, "해당 권한을 가진 유저가 없습니다."));
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.of(500, "서버 오류로 유저 목록을 불러오지 못했습니다."));
        }
    }
    ...
    public ResponseEntity<?> changeUserAuthority(
        ...
        try {
            ...
            if (result > 0) {
                return ResponseEntity.ok(ApiResponse.of(200, "유저 권한이 성공적으로 변경되었습니다."));
            } else {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(ApiResponse.of(404, "존재하지 않는 유저입니다."));
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.of(500, "서버 오류로 권한 변경에 실패했습니다."));
        }
    }
